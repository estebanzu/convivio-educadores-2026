...
timeline.innerHTML = eventData.itinerary
  .map((item, i) => {
    const isLeft = i % 2 === 0;
    return `
...
function validate() {
  ...
  eAtt.classList.add("hidden");
  if (!name || name.length < 3) {
    eName.textContent = "Ingresa tu nombre completo (mín. 3 caracteres).";
    eName.classList.remove("hidden");
    ok = false;
  }
  if (!att) {
    eAtt.textContent = "Selecciona si asistirás o no.";
    eAtt.classList.remove("hidden");
    ok = false;
  }
  return ok;
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
      ...
      if (webhook) {
        ...
        const waMsg = encodeURIComponent(
          `Hola! Confirmo para el Convivio de Educadores 2026:\n\n` +
            ...
            (data.restricciones
              ? `Alimentación: ${data.restricciones}\n`
              : "") +
        ...
        setTimeout(() => {
          if (
            confirm("¿Quieres enviar tu confirmación también por WhatsApp?")
          ) {
            window.open(
              `https://wa.me/50688830657?text=${waMsg}`,
              "_blank",
              "noopener",
            );
          }
    ...
    console.error(err);
    showError(
      "No se pudo enviar. Por favor usa el formulario oficial de Google o WhatsApp. Tus datos se guardaron localmente.",
    );
    // still store locally
